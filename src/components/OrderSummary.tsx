...
function OrderSummary(props: OrderSummaryProps) {
  ...
  useEffect(() => {
    setIsAccordionOpen(isContinueControlled);
  }, [isContinueControlled]);
  ...
  const calculateTotal = () => {
    return items.reduce(
      (total: number, item: CartItem) =>
        total + item.product_id.discountPrice * item.quantityToBuy,
      0
    );
  };
          ...
          {isAccordionOpen && (
            <div className="!pl-[15px] min-h-[250px] max-[550px]:!ml-[0px] md:min-h-[250px] w-full max-w-[775px] flex flex-col p-4 max-[550px]:h-auto">
              <div className="!py-[15px] w-full flex flex-col max-[550px]:!mt-[0px] max-[550px]:gap-[15px] max-[550px]:!px-[10px] cursor-pointer gap-y-4">
                <div>
                  {items.map((book: CartItem, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      <div className="w-full md:w-[75%] flex flex-col max-[550px]:justify-start">
                        ...
                        <p className="text-[15px] text-[#0A0102] max-sm:text-[12px]">
                          Rs. {book.product_id.discountPrice} x {book.quantityToBuy} ={' '}
                          {book.product_id.discountPrice * book.quantityToBuy}
                        </p>
                      </div>
                    </div>
                  ))}
