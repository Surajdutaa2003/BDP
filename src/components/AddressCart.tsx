...
type MyCartContainerProps = {
  isContinueClicked: boolean; // From MyCartContainer
  onContinueClick: () => void; // To OrderSummary
};

function AddressCart({ isContinueClicked, onContinueClick }: MyCartContainerProps) {
  const [isAccordionOpen, setIsAccordionOpen] = useState(false); // Starts closed

  useEffect(() => {
    setIsAccordionOpen(isContinueClicked); // Opens when MyCartContainer’s CONTINUE is clicked
  }, [isContinueClicked]);
  ...
  const handleContinue = () => {
    onContinueClick(); // Opens OrderSummary
    // Optionally collapse AddressCart:
              ...
              <div className="w-[100%] flex flex-col !mb-3 p-3 h-auto gap-y-5">
                <div className="w-full md:w-[80%] flex flex-col gap-y-5">
                  <div className="flex flex-col md:flex-row gap-y-5 md:gap-x-5 w-[100%]">
                    ...
                    <div className="flex flex-col justify-left !pr-[10px]">
                      <p className="text-[12px]">Mobile Number</p>
                      <input
                        type="text"
                        className="!py-[10px] !px-[15px] w-full md:w-[290px] !h-[50px] border border-[#DCDCDC] rounded-[3px]"
                        value=""
                  ...
                  <div className="flex flex-col gap-y-5">
                    ...
                    <div className="flex flex-col md:flex-row gap-y-5 md:gap-x-5 !pr-[10px]">
                      <div className="flex flex-col justify-left">
                        <p className="text-[12px]">City/Town</p>
                        <input
                          type="text"
